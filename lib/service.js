      ...
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: `Detect the language of the text and return ONLY a BCP-47 language tag (examples: en, ar, hi, fr, es, de, tr, zh, ja, pt-BR). If mixed, return dominant.\n\nTEXT:\n${key}`,
            },
          ],
        },
      ],
...
// ============================
// Localized fallback (IMPORTANT)
// ============================
function fallbackBusy(lang) {
  const L = String(lang || "auto").toLowerCase();
  if (L.startsWith("ar")) return "النظام تحت ضغط الآن. حاول مرة أخرى بعد لحظات.";
  if (L.startsWith("tr")) return "Sistem şu anda yoğun. Lütfen birkaç saniye sonra tekrar deneyin.";
  if (L.startsWith("es")) return "El sistema está ocupado ahora. Inténtalo de nuevo en unos segundos.";
  if (L.startsWith("fr")) return "Le système est chargé pour le moment. Réessaie dans quelques secondes.";
  if (L.startsWith("de")) return "Das System ist gerade ausgelastet. Bitte versuche es in ein paar Sekunden erneut.";
  if (L.startsWith("zh")) return "系统当前繁忙，请稍后再试。";
  if (L.startsWith("ja")) return "現在システムが混雑しています。少し待ってからもう一度お試しください。";
  return "The system is busy right now. Please try again in a moment.";
}

function fallbackBadAudio(lang) {
  const L = String(lang || "auto").toLowerCase();
  if (L.startsWith("ar")) return "الصوت غير واضح بما يكفي. سجّل 6–10 ثوانٍ قرب مصدر الصوت وبدون ريح قوية، ثم أعد الإرسال.";
  if (L.startsWith("tr")) return "Ses yeterince net değil. 6–10 saniye kaydedin, sesi çıkaran yere yaklaşın ve tekrar gönderin.";
  if (L.startsWith("es")) return "El audio no está lo bastante claro. Graba 6–10 segundos cerca de la fuente del ruido y vuelve a enviar.";
  if (L.startsWith("fr")) return "L’audio n’est pas assez clair. Enregistre 6–10 secondes près de la source du bruit puis renvoie-le.";
  if (L.startsWith("de")) return "Das Audio ist nicht klar genug. Nimm 6–10 Sekunden nahe an der Geräuschquelle auf und sende es erneut.";
  if (L.startsWith("zh")) return "音频不够清晰。请靠近噪音来源录制 6–10 秒后重新发送。";
  if (L.startsWith("ja")) return "音声が十分に明瞭ではありません。音源に近づいて 6〜10 秒録音し、再送してください。";
  return "The audio is not clear enough. Record 6–10 seconds close to the sound source and resend.";
}
...
`.trim();
  ...
  try {
    ...
    console.error("OPENAI TEXT FAIL:", e);
    return { reply: fallbackBusy(lang), language: lang };
  }
...
`.trim();
  ...
  try {
    ...
    console.error("OPENAI IMAGE FAIL:", e);
    return { reply: fallbackBusy(lang), language: lang };
  }
...
export async function diagnoseAudio({
  ...
  if (!audioBuffer || !(audioBuffer instanceof Buffer) || audioBuffer.length < 200) {
    const lang = await resolveLanguage({ preferredLanguage, message });
    return { reply: fallbackBadAudio(lang), language: lang, transcript: null };
  }
  ...
  // ✅ IMPORTANT: if transcript is empty, still reply in user's typed language
  ...
  const userPrompt = `
Language=${lang}.
Transcript=${transcriptText || "(no clear speech detected)"}.
FeaturesJSON=${JSON.stringify(features || { ok: false, reason: "no_features" })}.
...
Remember: do not invent speech if transcript is empty; use features as hints.
If transcript is empty AND features are weak, give a smart fallback message WITHOUT questions, still in the user's language.
`.trim();
  ...
  try {
    ...
    console.error("OPENAI AUDIO FAIL:", e);
    // ✅ reply in the user's language
    const reply = transcriptLooksEmpty ? fallbackBadAudio(lang) : fallbackBusy(lang);
    return { reply, language: lang, transcript: transcriptLooksEmpty ? null : transcriptText };
  }

  const raw = extractReplyFromResponse(resp) || "";
  const cleaned = normalizeOutput(raw);

  // ✅ If model replied empty for any reason, give localized fallback
  if (!cleaned) {
    const reply = transcriptLooksEmpty ? fallbackBadAudio(lang) : fallbackBusy(lang);
    return { reply, language: lang, transcript: transcriptLooksEmpty ? null : transcriptText };
  }
