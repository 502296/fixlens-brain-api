...
// ✅ UPDATED: if preferredLanguage exists (even like es-ES), prefer it strongly
async function resolveLanguage({ preferredLanguage, message, transcriptText }) {
  const p = _normalizeLang(preferredLanguage);

  // If user explicitly provided language like "es" / "fr" / "de" / "ar" => use it
  if (p && p !== "auto") return p;

  // Detect from typed message first
  ...
  // If everything fails, return "auto" (we will avoid auto later when possible)
  return "auto";
}
...
// IMAGE
// ============================
export async Fletcher function diagnoseImage({
  message,
  preferredLanguage,
  vehicleInfo,
  imageBuffer,
...
export async function diagnoseAudio({
  ...
  // ✅ Resolve lang early using preferredLanguage if available
  let lang = await resolveLanguage({ preferredLanguage, message, transcriptText: "" });

  // ✅ Avoid returning "auto" for audio: if we have preferredLanguage from header/body, use it
  const p = _normalizeLang(preferredLanguage);
  if ((lang === "auto" || !lang) && p && p !== "auto") lang = p;
  if (!lang) lang = "en";

  if (!audioBuffer || !(audioBuffer instanceof Buffer) || audioBuffer.length < 200) {
    // ✅ Generate the "invalid audio" message in user's language (no English-only)
    const resp = await openai.responses.create({
      model: MODEL_TEXT,
      instructions: buildDoctorMechanicInstruction(lang),
      input: [{
        ...
        content: [{
          type: "input_text",
          text: `Language=${lang}. The audio file is missing/too small. Write one short helpful paragraph telling the user to re-record 6–10 seconds, closer to the sound source, and avoid wind/background noise. No questions.`,
        }],
      }],
      temperature: 0.1,
    ...
    return { reply: normalizeOutput(extractReplyFromResponse(resp)), language: lang, transcript: null };
  }
  ...
  // ✅ Re-resolve language after transcript, but still prefer header/body language
  ...
  if (resolved && resolved !== "auto") lang = resolved;
  const p2 = _normalizeLang(preferredLanguage);
  if ((lang === "auto" || !lang) && p2 && p2 !== "auto") lang = p2;
  if (!lang) lang = "en";
...
Language=${lang}. Transcript=${transcriptText || "(no clear speech detected)"} FeaturesJSON=${JSON.stringify(
  ...
  } InternalMatchesJSON=${JSON.stringify(issues || [])}. Remember: do not invent speech if transcript is empty; use features as hints.
`.trim();
