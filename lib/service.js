...
- Reply in the user's language only (the language tag provided to you).
...
// ============================
// Language (FIXED: never trust transcript for language)
// ============================
...
// Accept-Language like: "ar,en-US;q=0.9,en;q=0.8"
function _acceptLanguageToTag(header) {
  const h = String(header || "").trim();
  if (!h) return null;
  const first = h.split(",")[0]?.trim() || "";
  const tag = first.split(";")[0]?.trim() || "";
  const norm = _normalizeLang(tag);
  return norm && norm !== "auto" ? norm : null;
}
...
/**
 * Resolve final language tag:
 * 1) preferredLanguage (if not auto)
 * 2) detect from typed message
 * 3) acceptLanguage header (if provided by server.js)
 * 4) default "en"
 *
 * NOTE: We intentionally do NOT use transcriptText to decide language
 * because Whisper can output random other languages for noisy engine audio.
 */
async function resolveLanguage({ preferredLanguage, message, acceptLanguage }) {
  ...
  const fromHeader = _acceptLanguageToTag(acceptLanguage);
...
User language tag: ${L}. You MUST reply in this language only.
...
export async function diagnoseText({
  message,
  preferredLanguage,
  acceptLanguage, // from server.js (optional)
  vehicleInfo,
  mode = "doctor",
}) {
  if (!message || !message.trim()) {
    const lang = await resolveLanguage({ preferredLanguage, message, acceptLanguage });
...
export async function diagnoseAudio({
  ...
  // Decide language EARLY (never from transcript)
  const lang = await resolveLanguage({ preferredLanguage, message, acceptLanguage });
  ...
  try {
    ...
    // Transcribe (for content only, NOT for language)
    try {
  ...
  // If transcript empty AND features too weak → localized fallback in user's language
