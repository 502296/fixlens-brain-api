...
function normalizeOutput(text) {
  ...
  t = t.replace(/\r\n/g, "\n");
  t = t.replace(/\n{3,}/g, "\n\n").trim();
  if (t.length > 9000) t = t.slice(0, 9000).trim(); // ✅ allow longer pro reports (still safe)
  return t;
}
...
// ============================
// ✅ Auto Report Depth + No-Price Guard (12/10 upgrade)
// ============================
...
function decideReportDepth({ message, vehicleInfo, transcriptText, audioFeatures }) {
  ...
  const heavy = _hasAny(textAll, [
    ...
    "شاحنة","سيمي","فريتلانر","كاسكاديا","ديريت","ريجن","دف","ديزل","TCM","ECU","CAN"
  ]);
...
function depthGuidance(depth) {
  if (depth === "deep") {
    return `
ReportDepth=DEEP
- Produce 20–35 numbered points when needed.
- Avoid repetition: do NOT restate the same check twice in different words.
- Use measurable targets when relevant (V / Ohms / PSI / °F).
- Prefer fast isolating tests (voltage-drop, wiggle-test, connector/ground checks, CAN resistance) for electrical/TCM/CAN cases.
- Give only 2–4 root-cause hypotheses, then a confirm/deny plan for each.
`.trim();
  }
  if (depth === "short") {
    return `
ReportDepth=SHORT
- Produce 10–16 numbered points.
- Tight and practical: top causes + quickest confirmation tests.
- Avoid theory unless it changes the next step.
`.trim();
  }
  return `
ReportDepth=NORMAL
- Produce 14–22 numbered points depending on complexity.
- Balanced: causes + confirm/deny steps, minimal repetition.
`.trim();
}
...
function pricingGuardText(lang, allowPricing) {
  if (allowPricing) return ""; // user asked
  const L = String(lang || "en").toLowerCase();
  if (L.startsWith("ar")) {
    return `
NoPricingRule:
- Do NOT mention any repair costs, price ranges, parts shopping suggestions, or labor estimates unless the user explicitly asks about cost.
`.trim();
  }
  return `
NoPricingRule:
- Do NOT mention repair costs, price ranges, parts shopping suggestions, or labor estimates unless the user explicitly asks about cost.
`.trim();
}
...
// ============================
// ✅ MEMORY + RELEVANCE GATE (14/10 upgrade)
// ============================

// Keep last N messages for session memory
const MAX_HISTORY_MESSAGES = Number(process.env.FIXLENS_MAX_HISTORY || 16);
...
function formatHistoryForPrompt(history) {
  ...
  // Keep it compact so we don't blow tokens
...
// Detect if the message actually contains a diagnostic request
function isDiagnosticMessage(message, vehicleInfo, transcriptText) {
  const t = `${message || ""}\n${vehicleInfo || ""}\n${transcriptText || ""}`.toLowerCase();

  // If user only says location / greeting / generic talk, treat as non-diagnostic
  const nonDiagOnly = [
    "i live in", "i’m in", "im in", "kentucky", "louisville",
    "hello", "hi", "thanks", "thank you", "good morning",
    "مرحبا", "هلا", "السلام", "صباح", "مساء", "شكرا", "حبيبي", "صديقي",
    "انا اعيش", "ساكن", "لويسفيل", "كنتاكي"
  ];

  // Diagnostic signals: codes, symptoms, warnings, behaviors
  const diagSignals = [
    "p0", "p1", "u0", "b0", "c0", // OBD prefixes
    "check engine", "cel", "misfire", "rough idle", "stall", "overheat", "smoke",
    "won't start", "no start", "crank", "knock", "vibration", "leak", "rpm",
    "derate", "regen", "dpf", "def", "aftertreatment",
    "لمبة", "تشيك", "كود", "رمز", "يطفى", "يقطع", "رجفة", "اهتزاز",
    "حرارة", "يسخن", "دخان", "ما يشتغل", "سلف", "طرق", "تسريب", "ضعف", "عزم",
    "مكيف", "تبريد", "ضغط زيت", "فرامل"
  ];

  const hasDiag = diagSignals.some((k) => t.includes(k));
  const looksNonDiag =
    nonDiagOnly.some((k) => t.includes(k)) &&
    !hasDiag &&
    (t.trim().length < 60);

  // If message is very short and no diagnostic signals -> non-diagnostic
  if (!hasDiag && t.trim().length < 35) return false;
  if (looksNonDiag) return false;

  // Otherwise likely diagnostic
  return true;
}

function relevanceGateText(lang, diagnostic) {
  if (diagnostic) return "";

  const L = String(lang || "en").toLowerCase();
  if (L.startsWith("ar")) {
    return `
RelevanceGateOverride:
- The user message is NOT a diagnostic request.
- Output ONLY 1–3 numbered lines:
  1) acknowledge briefly
  2) ask for the missing vehicle problem details (year + make/model + symptom + when it happens + codes if any)
  3) end with ONE question only
- Do NOT output a long checklist.
`.trim();
  }
...
- Output ONLY 1–3 numbered lines:
  ...
  2) ask for missing vehicle problem details (year + make/model + symptom + when it happens + codes if any)
  3) end with ONE question only
- Do NOT output a long checklist.
`.trim();
}
...
// TEXT
// ============================
export async function diagnoseText({ message, preferredLanguage, vehicleInfo, history = [], mode = "doctor" }) {
  ...
  const diagnostic = isDiagnosticMessage(message, vehicleInfo, "");
...
${relevanceGateText(lang, diagnostic)}

BehaviorGuidance:
- Be flexible: short when simple, detailed when complex.
- Avoid repetition. Each numbered line must add NEW information.
- Do NOT focus on prices unless the user asks.
- If location is needed for shops/junkyards, ask ONE location question inside the final single question.
- End with ONE question only.
`.trim();

  const maxTokens = depth === "deep" ? 1700 : depth === "short" ? 900 : 1200;
...
export async function diagnoseAudio({
  ...
  const historyText = formatHistoryForPrompt(history);
  const diagnostic = isDiagnosticMessage(message, vehicleInfo, transcriptText);
