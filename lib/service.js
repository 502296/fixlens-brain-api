...
// ✅ “لغة بيضاء” عالمية + العربية فصحى بسيطة
function buildBaseLanguageRules(lang) {
  ...
  const arabicRule =
    L === "ar"
      ? `
Arabic requirement:
- Use Modern Standard Arabic (فصحى مبسطة) that any Arabic speaker can understand.
- Avoid Iraqi / Gulf / Levant / Egyptian dialect words.
- Keep sentences clear, professional, and calm.
`.trim()
      : "";

  return `
Always reply in the user's language (${lang || "auto"}).
Use neutral "white language": simple, respectful, professional; not academic; not street slang.
${arabicRule}
`.trim();
}

// ✅ نفس أسلوبنا: تقرير واحد، بدون عناوين ولا نقاط
function buildSystemInstruction(lang) {
  return `
You are FixLens Auto — a world-class mechanic-style vehicle diagnostic assistant.
${buildBaseLanguageRules(lang)}
...
// ✅ Doctor Mode: نفس القواعد لكن أسلوب “دكتور ميكانيك عالمي”
function buildDoctorInstruction(lang) {
  return `
You are FixLens Doctor — an elite, world-class automotive diagnostic doctor.
${buildBaseLanguageRules(lang)}

Voice & style:
- Speak like a calm, highly experienced diagnostic specialist.
- Be precise, structured in paragraphs, and avoid exaggeration.
- Give a clear “most likely direction” + “what would confirm it” + “safe next actions”.

Important output rules:
- Write ONE single professional report as normal paragraphs.
- No headings, no bullet points, no numbering.
- No slang, no jokes, no dialect.
- Do not claim certainty. Use likelihood language.
- Ask ONLY 2–3 targeted questions inside the same report (not as a list).
`.trim();
}
...
// ============================
// WAV Decoder (بدون مكتبات)
// يدعم PCM 16bit + Float32
// ============================
...
function pickInstructions(lang, doctorMode) {
  return doctorMode ? buildDoctorInstruction(lang) : buildSystemInstruction(lang);
}

// ============================
// TEXT
// ============================
export async function diagnoseText({ message, preferredLanguage, vehicleInfo, doctorMode }) {
...
`.trim();
  ...
  const resp = await openai.responses.create({
    model: MODEL_TEXT,
    instructions: pickInstructions(lang, Boolean(doctorMode)),
    input: [{ role: "user", content: [{ type: "input_text", text: userPrompt }] }],
    temperature: doctorMode ? 0.22 : 0.35,
  });
...
`.trim();
  ...
  const resp = await openai.responses.create({
    ...
    temperature: doctorMode ? 0.18 : 0.3,
  });
...
`.trim();
  ...
  const resp = await openai.responses.create({
    ...
    instructions: pickInstructions(lang, Boolean(doctorMode)),
    input: [{ role: "user", content: [{ type: "input_text", text: userPrompt }] }],
    temperature: doctorMode ? 0.18 : 0.28,
  });
