...
// NEW: allow bullets + keep formatting clean (no excessive blank lines)
function normalizeOutput(text) {
  let t = String(text || "").trim();
  if (!t) return "";
  t = t.replace(/\r\n/g, "\n");
  t = t.replace(/\n{3,}/g, "\n\n").trim();
  // prevent super-long outputs just in case
...
async function resolveLanguage({ preferredLanguage, message, transcriptText }) {
  ...
  // IMPORTANT: if user sent "auto", don't force it — continue detection
