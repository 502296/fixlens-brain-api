...
- Reply in the user's language only. Never switch languages.
...
// ============================
// Language (ALL languages) + HARD ENFORCE
// ============================
          ...
          content: [
            {
              type: "input_text",
              text:
                "Detect the language of the text and return ONLY a BCP-47 tag (examples: en, ar, tr, ja, zh, es, fr). If mixed, return dominant.\n\nTEXT:\n" +
                key,
            },
          ],
        },
...
// ✅ translate/correct output to target language (NO extra content)
async function forceReplyLanguage(replyText, targetLang) {
  const target = _normalizeLang(targetLang) || "en";
  if (target === "auto") return replyText;

  const text = String(replyText || "").trim();
  if (!text) return text;

  // Fast check: if contains Arabic letters and target=ar => ok
  const targetPrimary = primaryLangTag(target);
  const detected = await detectLanguageWithAI(text);
  const detectedPrimary = primaryLangTag(detected || "");

  if (detectedPrimary === targetPrimary) return text;

  // Hard correction (translation) while preserving meaning and tone
  try {
    const resp = await openai.responses.create({
      model: MODEL_TEXT,
      temperature: 0.1,
          ...
          content: [
            {
              type: "input_text",
              text: `
Translate the following reply into ${target} ONLY.
Rules:
- Keep the same meaning.
- Keep the same mechanic tone (calm, confident, practical).
- Do NOT add new info.
- Do NOT add headings.
- Keep bullets if present (max 4).
...
`.trim(),
    ...
    const out = extractReplyFromResponse(resp) || text;
    return normalizeOutput(out) || text;
  } catch {
    return text;
  }
}
...
User language: ${L}.
If language is "auto", detect from the user's latest content and reply in that language.

STRICT REQUIREMENT:
- Output MUST be in ${L === "auto" ? "the detected user language" : L} only.
- If you start writing in another language, STOP and rewrite fully in the correct language.

Extra constraints:
- Target length: short and helpful.
...
`.trim();
  ...
  let raw = extractReplyFromResponse(resp) || "No reply generated.";
  raw = normalizeOutput(raw);

  // ✅ enforce language even for text (extra safety)
  const fixed = await forceReplyLanguage(raw, lang);

  return { reply: fixed, language: _normalizeLang(lang) || "en" };
}
...
IMPORTANT: Reply ONLY in Language=${lang}.
`.trim();
  ...
  // ✅ HARD FIX: if model replied in English, force it back to user's language
  const fixed = await forceReplyLanguage(raw, lang);
...
export async function diagnoseAudio({
  ...
  if (!audioBuffer || !(audioBuffer instanceof Buffer) || audioBuffer.length < 200) {
    const lang = await resolveLanguage({ preferredLanguage, message });
    const fb = localizedAudioFallback(lang);
    return { reply: fb, language: _normalizeLang(lang) || "en", transcript: null };
  }
...
`.trim();
  ...
  let raw = extractReplyFromResponse(resp) || localizedAudioFallback(lang);
  raw = normalizeOutput(raw);

  // ✅ HARD FIX: enforce same language as user
  const fixed = await forceReplyLanguage(raw, lang);

  return {
    reply: fixed,
    language: _normalizeLang(lang) || "en",
