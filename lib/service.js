...
// ✅ doctorPrompt.js exports: export const doctorPrompt = `...`
import { doctorPrompt as DOCTOR_PRO_PROMPT } from "./doctorPrompt.js";

// ============================
// OpenAI Client (SAFE: never crash server on missing key)
// ============================
function getApiKey() {
  return (
...
// ============================
// ✅ FIXLENS DOCTOR MECHANIC – PRO PROMPT
// ============================
...
// ============================
// ✅ Auto Report Depth + No-Price Guard
// ============================
...
export async function diagnoseText({
  ...
  // ✅ No key? return safe message, no crash
  const openai = getOpenAIClientOrNull();
  if (!openai) {
    const reply = lang.startsWith("ar")
      ? "FixLens Brain غير مفعّل الآن لأن مفتاح OpenAI غير موجود على السيرفر. أضف OPENAI_API_KEY في Railway Variables ثم جرّب مرة ثانية."
      : "FixLens Brain is not active because the OpenAI key is missing on the server. Add OPENAI_API_KEY in Railway Variables and try again.";
    return { reply, language: lang, ok: false };
  }
  ...
  if (!message || !message.trim()) {
    ...
    return { reply, language: lang === "auto" ? "en" : lang, ok: true };
  }
...
export async function diagnoseImage({
  ...
  if (!openai) {
    const reply = lang.startsWith("ar")
      ? "FixLens Brain غير مفعّل الآن لأن مفتاح OpenAI غير موجود على السيرفر. أضف OPENAI_API_KEY ثم جرّب مرة ثانية."
...
export async function diagnoseAudio({
  ...
  const openai = getOpenAIClientOrNull();
  const langPre = await resolveLanguage({ preferredLanguage, message });

  if (!openai) {
    const reply = langPre.startsWith("ar")
      ? "FixLens Brain غير مفعّل الآن لأن مفتاح OpenAI غير موجود على السيرفر. أضف OPENAI_API_KEY ثم جرّب مرة ثانية."
      : "FixLens Brain is not active because the OpenAI key is missing on the server. Add OPENAI_API_KEY and try again.";
    return { reply, language: langPre, transcript: null, ok: false };
  }
