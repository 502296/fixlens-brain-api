...
function loadDataHub() {
  ...
  _dataCache = {
    ...
    files,
    byFile,
    totalItems: Object.values(byFile).reduce((n, arr) => n + (Array.isArray(arr) ? arr.length : 0), 0),
  };
...
function _scoreIssue(msgTokens, issue) {
  ...
  // Token overlap bonus (helps when phrasing differs)
...
You always reply in the user's language (${L}). If language is "auto", detect from the user's latest content and reply in that language.

Style rules:
- Use "white language": neutral, professional, globally understandable.
- No slang, no regional dialects, no street tone, no academic tone.
- Do not ask questions. Do not request more details as questions.

Output rules:
- Write ONE single professional mechanic-style report as normal paragraphs.
- No headings, no bullet points, no numbering.
- Be decisive and structured: prioritize the most likely root causes first, then less likely ones, then the safest next actions.
- Use likelihood language only when needed (likely / possible / less likely). Avoid repeating it.
- Use the provided internal database matches as primary evidence; do not ignore them.
...
Internal database matches (ranked):
${JSON.stringify(issues || [], null, 2)}

Final rule: Reply strictly in "${lang}" with the Doctor Mechanic style rules.
`.trim();
...
Analyze the image and the context. Use the internal database matches if relevant.
...
AUDIO TRANSCRIPTION (may be empty if no clear speech):
${transcriptText || "(no clear speech detected)"}

AUDIO SIGNAL FEATURES (waveform-derived clues):
...
Rules:
- Do not invent exact words if transcription is empty.
- Do not claim certainty from sound alone.
- Use the waveform features as hints and give a smart fallback message if needed (no questions).
- Reply strictly in "${lang}" with the Doctor Mechanic style rules.
`.trim();
