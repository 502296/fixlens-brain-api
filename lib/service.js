...
async function enforceLanguage(outputText, targetLang) {
  ...
  const target = _normalizeLang(targetLang);
  if (!target || target === "auto") return out; // ما نقدر نترجم إذا الهدف auto
...
User language: ${L}.
Hard rule: You MUST output in "${L}" only. If you accidentally output another language, rewrite it into "${L}".
