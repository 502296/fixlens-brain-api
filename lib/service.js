...
// ============================
// ✅ Conversation History (Client-side memory, server stateless)
// ============================
...
// Accept multiple formats from Flutter:
// A) [{isUser:true,text:"..."}, {isUser:false,text:"..."}]
// B) [{role:"user",text:"..."}, {role:"assistant",text:"..."}]
// C) [{role:"user",content:"..."} ...]
function normalizeHistory(historyRaw) {
  const h = Array.isArray(historyRaw) ? historyRaw : [];
  const out = [];
  ...
  for (const item of h) {
    ...
    if (typeof item.isUser === "boolean") {
      const role = item.isUser ? "user" : "assistant";
      const text = _safeText(item.text ?? item.content ?? "");
    ...
    const text = _safeText(item.text ?? item.content ?? item.message ?? "");
...
function buildConversationSoFar(historyRaw, limitPairs = 8) {
  const h = normalizeHistory(historyRaw);

  // Keep last N turns (user+assistant pairs). We approximate by keeping last (limitPairs*2 + 2)
  const maxItems = Math.max(6, limitPairs * 2 + 2);
  const tail = h.slice(-maxItems);
...
async function resolveLanguage({
  ...
  // ✅ Highest priority: explicit preferredLanguage (from Flutter)
  const p = _normalizeLang(preferredLanguage);
  if (p && p !== "auto") return p;

  // ✅ Next: language from headers (x-fixlens-lang)
  const h = _normalizeLang(headerLanguage);
  if (h && h !== "auto") return h;

  // ✅ If we have conversationSoFar, detect from that first (better than last single message)
  const convo = (conversationSoFar || "").trim();
  if (convo.length >= 10) {
    const fromConvo = await detectLanguageWithAI(convo);
    if (fromConvo && fromConvo !== "auto") return fromConvo;
  }
...
export async function diagnoseText({
  ...
  if (!message || !message.trim()) {
    const lang = _normalizeLang(preferredLanguage) || _normalizeLang(headerLanguage) || "en";
  ...
  const conversationSoFar = buildConversationSoFar(history, 8);
...
BehaviorGuidance:
- This is a continuing conversation. Use ConversationSoFar as ground truth.
- Do NOT ask for make/model/location/symptom details if already present in VehicleInfo or ConversationSoFar.
- If the user message is only a short answer (e.g., a city name), continue from the prior question instead of resetting.
...
BehaviorGuidance:
- This is a continuing conversation. Use ConversationSoFar as ground truth.
- Do NOT ask for details already present in VehicleInfo or ConversationSoFar.
- Be flexible: short when simple, detailed when complex.
- Avoid repetition. Each numbered line must add NEW information.
- Do NOT focus on prices unless the user asks.
...
export async function diagnoseAudio({
  ...
  if (!audioBuffer || !(audioBuffer instanceof Buffer) || audioBuffer.length < 200) {
    const lang = await resolveLanguage({
      ...
      message,
      transcriptText: "",
      conversationSoFar: buildConversationSoFar(history, 8),
    });
