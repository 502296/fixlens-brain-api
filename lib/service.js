...
// ✅ IMPORTANT: your doctorPrompt.js exports `doctorPrompt`, not DOCTOR_PRO_PROMPT
import { doctorPrompt as DOCTOR_PRO_PROMPT } from "./doctorPrompt.js";

// ============================
// OpenAI client (SAFE: no crash if missing key)
// ============================
function getApiKey() {
  const k =
...
function getOpenAIClientOrNull() {
  ...
  try {
    ...
    console.error("OPENAI INIT ERROR:", e?.message || String(e));
    return null;
  }
}
...
function buildDoctorMechanicProPrompt(langLocked) {
  const L = String(langLocked || "en").trim() || "en";
...
// ============================
// Helpers: extracting / normalizing output
// ============================
...
// ============================
// Retry / Safety wrapper (prevents 502 chain reaction)
// ============================
function sleep(ms) {
...
function isRetryableError(err) {
  ...
  // OpenAI SDK sometimes exposes status code
...
async function callOpenAIWithRetry(fn, { tries = 3 } = {}) {
  let lastErr = null;
  for (let i = 0; i < tries; i++) {
    try {
      ...
      lastErr = e;
      const retry = isRetryableError(e);
      console.error("OPENAI CALL ERROR:", e?.status || "", e?.message || e);
      if (!retry || i === tries - 1) break;
      await sleep(450 + i * 600);
    }
  }
  throw lastErr;
}
...
// ============================
// Language (BCP-47) + detection/translation
// ============================
...
- Avoid repetition: do NOT restate the same check twice.
- Use measurable targets when relevant (V / Ohms / PSI / °F).
- Prefer fast isolating tests (voltage-drop, wiggle-test, connector/ground checks, CAN resistance) for electrical/TCM/CAN.
...
function localizedKeyMissing(lang) {
  const L = String(lang || "en").toLowerCase();
  if (L.startsWith("ar")) {
    return 'FixLens Brain غير مُفعّل لأن مفتاح OpenAI غير موجود على السيرفر. أضف OPENAI_API_KEY في Railway Variables ثم أعد التشغيل.';
  }
...
export async function diagnoseText({
  ...
  // If OpenAI key missing, do NOT crash
  if (!openai) {
    const lang = (await resolveLanguage(null, { preferredLanguage, message })) || "en";
    return { reply: localizedKeyMissing(lang), language: lang, ok: false };
  }
...
BehaviorGuidance:
- Use ConversationHistory as the source of truth for already-provided info (year/make/model/location/symptoms).
- NEVER ask for year/make/model/location if it already exists in ConversationHistory.
...
`.trim();
  ...
  const fixed = await enforceLanguage(openai, normalizeOutput(raw), lang);
...
export async function diagnoseImage({
  ...
  const lang = await resolveLanguage(openai, { preferredLanguage, message });
  const issues = findRelevantIssuesFromData(String(message || "").trim());
...
ContextText=${(String(message || "").trim() || "(no text provided)")}
${vehicleInfo ? `VehicleInfo=${String(vehicleInfo).trim()}` : ""}
InternalMatchesJSON=${JSON.stringify(issues || [])}
...
export async function diagnoseAudio({
  ...
  // quick guard for empty uploads
  if (!audioBuffer || !(audioBuffer instanceof Buffer) || audioBuffer.length < 200) {
    const openaiTmp = getOpenAIClientOrNull();
    const lang = await resolveLanguage(openaiTmp, { preferredLanguage, message });
    return { reply: localizedAudioFallback(lang), language: lang, transcript: null, ok: false };
  }

  const openai = getOpenAIClientOrNull();
  const langPre = await resolveLanguage(openai, { preferredLanguage, message });

  if (!openai) {
    return { reply: localizedKeyMissing(langPre), language: langPre, transcript: null, ok: false };
  }
  ...
  try {
    ...
    try {
      ...
      transcriptText = String(transcription?.text || "").trim();
    } catch {
      transcriptText = "";
    }
  } finally {
  ...
  if (transcriptLooksEmpty && featuresTooWeak) {
    return { reply: localizedAudioFallback(lang), language: lang, transcript: null, ok: true };
  }
...
TypedContext=${(String(message || "").trim() || "(no text provided)")}
${vehicleInfo ? `VehicleInfo=${String(vehicleInfo).trim()}` : ""}
InternalMatchesJSON=${JSON.stringify(issues || [])}
