...
// ✅ مهم: نحن لا نريد "auto" كناتج نهائي أبداً
function _finalizeLang(code) {
  const c = _normalizeLang(code);
...
// ============================
// ✅ هنا الإصلاح الحقيقي: نجبر اللغة بشكل صريح وقوي
function buildDoctorMechanicInstruction(lang) {
  const finalLang = _finalizeLang(lang);
  const label = languageLabel(finalLang);

  return `
${SYSTEM_PROMPT_GLOBAL}

LANGUAGE ENFORCEMENT (must follow strictly):
- The user's language is: ${finalLang} (${label}).
- Respond ONLY in ${label}.
- Do NOT use English or any other language at all.
- Do NOT mix languages.
- If you must output numbers or car codes, keep surrounding words in ${label}.

Extra constraints:
- No questions.
- No headings.
- Prefer 3–6 sentences OR up to 4 bullet lines "•" if clearer.
`.trim();
}
...
export async function diagnoseText({
  ...
  if (!message || !message.trim()) {
    const lang = _finalizeLang(preferredLanguage);
    const reply =
      lang.startsWith("ar")
...
Respond using the rules. Do not ask questions.
`.trim();
...
Analyze the image carefully and respond using the rules. No questions.
`.trim();
...
export async function diagnoseAudio({
  ...
  // ✅ إذا transcript فارغ + features ضعيفة → fallback بلغته (بدون أسئلة)
...
- Use AudioFeaturesJSON + TypedContext to diagnose mechanical sound.
- No questions.
`.trim();
