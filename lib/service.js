...
async function resolveLanguage({ preferredLanguage, message, transcriptText }) {
  ...
  // ✅ hard fallback (no auto)
  return "en";
}

// ============================
// ✅ Language Lock (force output language)
// ============================
        ...
        content: [
          {
            ...
            text:
              ...
              `Do NOT add new info. Do NOT add headings. Do NOT ask questions.\n` +
...
async function enforceLanguage(outputText, targetLang) {
  ...
  if (target === "auto") return out; // should not happen now
  ...
  // ✅ force translate to user language
...
User language: ${locked}.
Hard rule: You MUST output in "${locked}" only. If you accidentally output another language, rewrite it into "${locked}".
