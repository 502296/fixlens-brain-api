...
// ============================
// GLOBAL SYSTEM PROMPT (UPDATED - DO NOT REMOVE)
// ============================
// ✅ Changes:
// - Better non-English quality: "exact user's language, no mixing"
// - Allow "smart inline bullets" using " • " only (NO line breaks)
// - Shorter replies (no boring long essays)
...
- Reply in the user's language only (do not mix English terms). If a technical term is needed, add a short local-language explanation in parentheses.
...
- No numbering.
- No multi-line lists.
- If bullets help clarity, use INLINE bullets only, separated by " • " (single paragraph, no line breaks).
- Target output length: 2–5 sentences max. If bullets are used, keep them to 2–4 bullets inline.
`.trim();
...
// ✅ force single paragraph + also normalize weird bullet newlines
function enforceSingleParagraph(text) {
  const t = String(text || "").trim();
  if (!t) return "";
  // convert multiple lines to one line
  let s = t.replace(/\s*\n+\s*/g, " ").replace(/\s{2,}/g, " ").trim();
  // normalize bullets: if model used "•" with extra spacing/newlines
  s = s.replace(/\s*•\s*/g, " • ").replace(/\s{2,}/g, " ").trim();
  // avoid starting with a bullet separator
  s = s.replace(/^•\s*/g, "");
  return s;
}
...
// ============================
// WAV Decoder + audio features (كما هو عندك)
// ============================
...
// ============================
// Doctor Mechanic Instruction (UPDATED)
// ============================
...
Audio rule:
- If audio speech is unclear, do not guess speech content; use waveform features as clues and give a smart fallback without questions.
`.trim();
}
...
export async function diagnoseText({
  ...
  const userPrompt = `
Language=${lang}. User message: ${(message || "").trim()}${vehicleInfo ? ` Vehicle info: ${String(vehicleInfo).trim()}` : ""} Internal matches JSON: ${JSON.stringify(issues || [])}
`.trim();
...
export async function diagnoseImage({
  ...
  const userPrompt = `
Language=${lang}. Context text: ${((message || "").trim() || "(no text provided)")}${vehicleInfo ? ` Vehicle info: ${String(vehicleInfo).trim()}` : ""} Internal matches JSON: ${JSON.stringify(issues || [])}. Analyze the image carefully and respond using the rules.
`.trim();
...
export async function diagnoseAudio({
  ...
  const userPrompt = `
Language=${lang}. Transcript=${transcriptText || "(no clear speech detected)"} FeaturesJSON=${JSON.stringify(features || { ok: false, reason: "no_features" })} TypedContext=${((message || "").trim() || "(no text provided)")}${vehicleInfo ? ` VehicleInfo=${String(vehicleInfo).trim()}` : ""} InternalMatchesJSON=${JSON.stringify(issues || [])}. Remember: do not invent speech if transcript is empty; use features as hints; keep it short; inline bullets allowed with " • " only.
`.trim();
