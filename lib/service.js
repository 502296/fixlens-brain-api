...
// ============================
// ✅ Safe OpenAI call wrapper (NO crash)
// ============================
async function safeResponsesCreate(openai, payload, { timeoutMs = 120000 } = {}) {
  // If OpenAI is missing, fail gracefully
  ...
  try {
    const resp = await openai.responses.create({
      ...payload,
      // SDK supports fetch under the hood; abort works in Node 18+ typically
...
function formatHistoryForPrompt(history) {
  ...
  return h.map((m) => `${m.role === "user" ? "User" : "Assistant"}: ${m.text}`).join("\n");
}
...
export async function diagnoseText({
  ...
  // Graceful key handling (NO crash)
...
export async function diagnoseAudio({
  ...
  try {
    ...
    // Convert + features + transcript
    try {
      await convertToWav16kMono(tmpIn, tmpWav);
    } catch (e) {
      // If ffmpeg fails, fallback politely
      const lang = await resolveLanguage(openai, { preferredLanguage, message });
      return {
