...
- Reply in the user's language only, with natural native phrasing (not literal translation).
- Output MUST be ONE single paragraph only (no line breaks).
- No headings.
- Bullet points are allowed ONLY as inline bullets using the character "•" inside the same paragraph (no new lines), and only when listing multiple items.
...
function _scoreIssue(msgTokens, issue) {
  ...
  for (const p of all) {
    if (!p) continue;
    if (msg.includes(p))
      score += Math.min(6, 2 + Math.floor(p.split(" ").length / 2));
  }
...
- Target length: 4 to 7 sentences maximum.
- If you need to list multiple causes or actions, use inline bullets like: "… • item 1 • item 2 • item 3" (same paragraph).
- Max inline bullets: 4 total. Do not exceed.
...
export async function diagnoseText({
  ...
  // cleaner prompt (keeps same info, but avoids headings-driven long reports)
  const userPrompt = `
...
Language=${lang}. Transcript=${transcriptText || "(no clear speech detected)"} FeaturesJSON=${JSON.stringify(
  ...
  } InternalMatchesJSON=${JSON.stringify(
    issues || []
  )}. Remember: do not invent speech if transcript is empty; use features as hints; single paragraph only.
`.trim();
