...
- Treat the conversation as CONTINUOUS. Never reset context if ConversationHistory is provided.
`.trim();
}
...
async function resolveLanguage({ preferredLanguage, message, transcriptText }) {
  ...
  // ✅ If user picked a language, DO NOT override
...
function _looksArabic(t) {
  ...
  const m = s.match(/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF]/g);
  return (m ? m.length : 0) >= 10;
}
...
async function enforceLanguage(outputText, targetLang) {
  ...
  // ✅ Strong heuristic: if target is EN but Arabic appears, translate
  if (_primaryLang(target) === "en" && _looksArabic(out)) {
...
// ============================
// Expects conversation = [{ role: "user"|"assistant", text: "..." }, ...]
function formatConversationHistory(conversation) {
  ...
  const safe = conversation
    ...
    .map((x) => ({
      role: String(x.role || "").toLowerCase().trim(),
      text: String(x.text || x.content || "").trim(),
    }))
    .filter((x) => (x.role === "user" || x.role === "assistant") && x.text);

  if (!safe.length) return "";

  // keep last 12 turns max (24 lines worst-case)
  const tail = safe.slice(-12);
...
export async function diagnoseText({
  ...
  preferredLanguage,
  vehicleInfo,
  conversation, // ✅ NEW (stateless memory)
  mode = "doctor",
}) {
...
BehaviorGuidance:
- Continue the SAME conversation if ConversationHistory is provided.
- Do NOT restart or ask generic intake questions that were already answered.
