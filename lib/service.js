...
// ============================
// GLOBAL SYSTEM PROMPT (NEW - DO NOT REMOVE)
// ============================
...
// NEW: force one-paragraph output (no line breaks)
...
export async function diagnoseText({
  ...
  // NEW: cleaner prompt (keeps same info, but avoids headings-driven long reports)
  const userPrompt = `
...
`.trim();
  ...
  const resp = await openai.responses.create({
    ...
    temperature: 0.15, // NEW: slightly lower for less repetition
    max_output_tokens: 260, // NEW: keeps answers short & professional
  });
...
export async function diagnoseAudio({
  ...
  // NEW: cleaner prompt + keeps all info, but prevents long structured reports
  const userPrompt = `
Language=${lang}. Transcript=${transcriptText || "(no clear speech detected)"} FeaturesJSON=${JSON.stringify(features || { ok: false, reason: "no_features" })} TypedContext=${((message || "").trim() || "(no text provided)")} ${vehicleInfo ? ` VehicleInfo=${String(vehicleInfo).trim()}` : ""} InternalMatchesJSON=${JSON.stringify(issues || [])}. Remember: do not invent speech if transcript is empty; use features as hints; single paragraph only.
`.trim();
