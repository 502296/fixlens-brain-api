...
/**
 * Hardening for Railway:
 * - never crash process
 * - log errors without killing container
 */
process.on("unhandledRejection", (err) => {
  console.error("UNHANDLED_REJECTION:", err?.message || err, err?.stack || "");
});
process.on("uncaughtException", (err) => {
  console.error("UNCAUGHT_EXCEPTION:", err?.message || err, err?.stack || "");
});
...
// Text => GPT-5.1 (or your env override)
// Vision => GPT-4o
...
// Optional: disable audio features if you want ultra-light server
const ENABLE_AUDIO_FEATURES = String(process.env.ENABLE_AUDIO_FEATURES || "1") !== "0";
...
// IMPORTANT FIX: signal must be passed as OPTIONS (2nd arg), not inside body.
// ============================

// Responses API
async function safeResponsesCreate(openai, body, { timeoutMs = 120000 } = {}) {
  ...
  try {
    // ✅ correct usage: (body, options)
    const resp = await openai.responses.create(body, { signal: controller.signal });
    return resp;
  } catch (e) {
...
// Chat Completions
async function safeChatCompletionsCreate(openai, body, { timeoutMs = 120000 } = {}) {
  if (!openai) return { __error: "missing_key" };
  ...
  try {
    // ✅ correct usage: (body, options)
    const resp = await openai.chat.completions.create(body, { signal: controller.signal });
    return resp;
  } catch (e) {
    return { __error: e?.message || String(e) };
  } finally {
...
// ⚠️ lighter DFT (smaller size) to avoid CPU spikes on Railway
...
async function analyzeWavFeatures(wavPath) {
  if (!ENABLE_AUDIO_FEATURES) return { ok: false, reason: "disabled" };
  ...
  // keep max 8s to reduce CPU
  const maxSamples = Math.min(ch.length, sampleRate * 8);
  ...
  // smaller FFT size for performance
...
`.trim();
    ...
    const maxTokens = depth === "deep" ? 1700 : 1250;
