...
function normalizeMode(mode) {
  ...
  if (m === "doctor" || m === "fixlens_doctor" || m === "doctor_mode") return "doctor";
  return "auto"; // default
}

// ✅ لغة بيضاء + دكتور ميكانيك عند تفعيل doctor mode
function buildSystemInstruction(lang, mode) {
...
- Use neutral, standard language (no regional slang/dialects).
- If the language is Arabic, write in clear Modern Standard Arabic (فصحى مبسطة) without Iraqi or any regional dialect.

Output rules:
- Write ONE single professional report as normal paragraphs.
- No headings, no bullet points, no numbering.
- Be confident and helpful from the first sentence, like an expert mechanic.
...
`.trim();
  ...
  if (finalMode !== "doctor") return base;

  const doctorAddon = `
Doctor Mode style:
- Sound like a "mechanic doctor" with calm authority and empathy.
- Explain what you suspect and why in simple terms.
- Prioritize safety: if there is any risk of engine damage or unsafe driving, say so clearly.
- Give a practical next step the user can do now + what a workshop should check.
`.trim();
...
// TEXT
// ============================
export async function diagnoseText({ message, preferredLanguage, vehicleInfo, mode }) {
...
export async function diagnoseImage({
  ...
  // ✅ Guardrail: لو الصورة مو سيارة/جزء سيارة، لا “يفتي”
  const userPrompt = `
You must first confirm whether the image is vehicle-related (car, engine, dashboard, part, leak, tire, brake, etc.).
If it is NOT vehicle-related, say clearly you cannot diagnose it and ask the user to send a car/part photo. Do not give unrelated analysis.
...
export async function diagnoseAudio({
  ...
  try {
    ...
    try { features = await analyzeWavFeatures(tmpWav); }
    catch { features = { ok: false, reason: "feature_extract_failed" }; }
  ...
  const userPrompt = `
You received an audio clip. This can be engine/mechanical noise or a spoken description.

TRANSCRIPTION (may be empty):
${transcriptText || "(no clear speech detected)"}

AUDIO SIGNAL FEATURES (use as clues, not certainty):
${JSON.stringify(features || { ok: false, reason: "no_features" }, null, 2)}

CONTEXT (typed message):
${(message || "").trim() || "(no text provided)"}
...
Rules:
- Do NOT claim you identified an exact component from sound alone.
- Use the signal features as hints (impulsive vs rumble vs whine) + context.
- Ask 2–3 targeted questions inside the same report.
- If both transcription and typed context are empty, request a better recording:
  record 8–10 seconds close to the source, then speak 5 seconds describing when it happens, in the same clip.
`.trim();
